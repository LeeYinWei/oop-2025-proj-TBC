...
def load_electric_images():
    ...
    for i in range(1, 11, 2):  # 假設有 shockwave0.png ~ shockwave4.png
            img = pygame.image.load(f"electric/electric_{i}.png").convert_alpha()
            if scale != 1.0:
...
def load_gas_images():
    ...
    for i in range(1, 11, 2):  # 假設有 shockwave0.png ~ shockwave4.png
        img = pygame.image.load(f"gas/Explosion_{i}.png").convert_alpha()
        if scale != 1.0:
            new_size = (int(img.get_width() * scale), int(img.get_height() * scale))
            img = pygame.transform.scale(img, new_size)
        image_copy = img.copy()
...
def load_smoke_images():
    ...
    for i in range(1, 25, 5):  # 假設有 shockwave0.png ~ shockwave4.png
        img = pygame.image.load(f"smoke/smoke_frames/processed_frame_{i}.png").convert_alpha()
        if scale != 1.0:
            new_size = (int(img.get_width() * scale), int(img.get_height() * scale))
            img = pygame.transform.scale(img, new_size)
        image_copy = img.copy()
